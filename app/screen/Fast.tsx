...
export default function Fast() {
  ...
  const handleCreateSchedule = async () => {
    ...
    if (!scheduleDate || !scheduleTime || !scheduleJobs || !scheduleLocation || !pin) {
      ...
      try {
        ...
        try {
          const createFastData = await axios.post(`https://friendszone.app/api/fast`, {
            userId: userData?.id,
            time: scheduleTime,
            date: scheduleDate,
  ...
  const checkFast = async () => {
    try {
      setLoading(true);
      const checkFast = await axios.get(`https://friendszone.app/api/fast-check/${userData?.id}`)

      if (checkFast.status == 200) {
        if (checkFast.data.status == 200) {
          if (checkFast.data.data.code == "TIME_LEFT") {
            const time_left = checkFast.data.data.time_left;
            Alert.alert("แจ้งเตือน", `คุณสามารถสร้างรายการนัดหมายใหม่ได้ในอีก ${time_left} นาที`, [
              {
                text: 'กลับไปหน้าแรก',
  ...
  const fast_service_rate = async () => {
    try {
      const price = await axios.get('https://friendszone.app/api/fast-service')
  ...
  const createScheduleReal = async (member: MembersDB, total: number) => {
    ...
    let [day, month, year] = fastRequest.date.split("/").map(Number);
    ...
    try {
      ...
      if (response.data.status != 200) {
        ...
        try {
          await axios.put(`https://friendszone.app/api/fast/${fastRequest}`, {
            id: fastRequest.id,
          }, {
            headers: {
              'Content-Type': 'application/json',
  ...
  const cancelFastRequest = async () => {
    try {
      const res = await axios.put(`https://friendszone.app/api/fast`, {
        id: fastId
      }, {
        headers: {
          Authorization: `All ${userData?.token}`,
      ...
      if (res.status == 200) {
        ...
        Alert.alert('เกิดข้อผิดพลาด', 'ไม่สามารถยกเลิกการค้นหาได้', [{ text: 'OK' }]);
      }
    } catch (error) {
      Alert.alert('เกิดข้อผิดพลาด', 'ไม่สามารถยกเลิกการค้นหาได้', [{ text: 'OK' }]);
    }
  ...
  const fetchFastRequest = async () => {
    try {
      const res = await axios.get(`https://friendszone.app/api/fast/${userData?.id}?status=pending`, {
        headers: {
          Authorization: `All ${userData?.token}`,
          "Content-Type": "application/json"
        }
            ...
            firstLoading ? (
              <>
                <StyledView className="flex-1 justify-center items-center">
                  ...
                  <StyledText className="font-custom text-black dark:text-white text-lg mt-2">กำลังตรวจสอบ</StyledText>
                </StyledView>
              </>
